...
const AI_MODEL_URL = process.env.AI_MODEL_URL || 'http://localhost:8023';
...
app.post('/api/analyze-text', validateRequest, async (request, response) => {
	...
	try {
		...
		const apiResponse = await axios.post(
			...
			{
				text: text,
				queryType: query_type || 'mental health',
				model_size: '8b',
		...
		if (!data.is_mental_health) {
			return response.status(400).json({
				error: 'Content is not Mental Health related',
