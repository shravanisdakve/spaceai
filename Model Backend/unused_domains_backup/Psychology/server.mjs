...
const AI_MODEL_URL = process.env.AI_MODEL_URL || 'http://localhost:8005';
...
app.post('/api/analyze-text', validateRequest, async (request, response) => {
	...
	try {
		...
		const apiResponse = await axios.post(
			...
			{
				text: text,
				queryType: query_type || 'psychology',
				model_size: '8b',
		...
		if (!data.is_psychology) {
			return response.status(400).json({
				error: 'Content is not Psychology related',
